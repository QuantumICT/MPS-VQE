...
function run_task(comm :: MPI.Comm)
	...
	if rank == root
		...
		@time_redirect out_file p["hamiltonian_qubit_op"] = MPI.Scatterv(H, root, comm)
		println(out_file, "Distributing Hamiltonian Operator finished!")
...
function main()
	...
	new_comm = MPI.Comm_split(old_comm, color, old_rank % group_nprocs)
